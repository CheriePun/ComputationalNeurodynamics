...
class QIFNetwork:
  ...
  def NeuronUpdate(self, i, t):
    ...
    for k in xrange(int(1/dt)):
      ...
      self.layer[i].v += dt*((self.layer[i].a*(self.layer[i].vr-v)*(self.layer[i].vc-v) + self.layer[i].R*self.layer[i].I)/self.layer[i].tau)
...
class QIFLayer:
  """
  Layer of Quadratic integrate-and-fire neurons to be used inside an QIFNetwork.
  """
